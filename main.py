...
def move_left(cursor_y, cursor_x, editor_y, state):
    ...
    if (
        is_cursor_at_line_beginning
        and is_cursor_at_editor_top
        and editor_y == 0
    ):
...
def move_right(cursor_y, cursor_x, editor_y, state):
    ...
    is_cursor_at_line_end = (
        cursor_x == len(curr_line_text) or cursor_x == EDITOR_WIDTH - 1
    )
    ...
    is_cursor_at_file_end = (
        is_cursor_at_line_end
        and is_cursor_at_editor_bottom
        and cursor_y + editor_y == len(state) - 1
    )
...
def main(scr):
    ...
    while True:
        ...
        # CTRL + F
            ...
            statusbar.clear()
            statusbar.addstr(
                "Waiting for a command character", BLACK_AND_YELLOW
            )
            key = statusbar.getch()
            # f - find
            if key == 102:
            ...
            if char == "\n":
                ...
                if x != 0:
                    state[y + editor_y] = (
                        state[y + editor_y][: x - 1] + state[y + editor_y][x:]
                    )
                    new_x = x - 1
                else:
                    if (
                        len(state[y + editor_y]) == 0
                        and not cursor_at_file_beginning
                    ):
                        del state[y + editor_y]
                        new_y = y - 1
                        ...
                        state[y + editor_y - 1] = (
                            state[y + editor_y - 1] + state[y + editor_y]
                        )
                        del state[y + editor_y]
                        new_x = len(state[y + editor_y - 1])
                        new_y = y - 1
                ...
                state[y + editor_y] = (
                    state[y + editor_y][:x] + char + state[y + editor_y][x:]
                )
